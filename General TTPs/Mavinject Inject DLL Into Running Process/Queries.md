...
## TTP hunt
```
| tstats `security_content_summariesonly` count values(Processes.process) as process values(Processes.parent_process) as parent_process min(_time) as firstTime max(_time) as lastTime from datamodel=Endpoint.Processes where Processes.process IN ("*/INJECTRUNNING*") NOT Processes.parent_process_name=AppVClient.exe by Processes.user Processes.process_name Processes.parent_process_name Processes.dest
...
## TTP hunt
```
event_platform=win event_simpleName=ProcessRollup2 NOT ParentBaseFileName=AppVClient.exe CommandLine IN ("*/INJECTRUNNING*") 
